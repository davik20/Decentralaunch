...
const TokenList = () => {
  ...
  useEffect(() => {
    ...
    const getTokenLocks = new Promise((resolve, reject) => {
      FireSalesLockerContract.getPastEvents(
        "LockCreated",
        {
          fromBlock: "earliest",
          filter: {
        ...
        .then((events: any) => {
          let locks = events.map((item: any) => {
            console.log(item);
            let tokenAmount = item.returnValues.tokenAmount;
            let decimals = item.returnValues.decimals;
          ...
          // let items = "123444".split("").map((item, index, array) => {
          //   return locks[0];
          // });
          resolve(locks);
        })
  ...
  return (
    <div>
      ...
      <div>
        ...
        {locks.data.length === 0 && locks.status === "resolved" && (
            <NoData />
          )}
        ...
        <TabPanel hidden={selectedTab !== "myLocks"}>
          ...
          {myLocks.data.length === 0 && myLocks.status === "resolved" && account && !chainIdError && (
            <NoData />
          )}
